      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-[28px] lg:gap-[44px] items-start">
        ...
        <div className="rounded-[14px] overflow-hidden font-mono shadow-[0_28px_72px_-16px_rgba(0,0,0,.75)]" style={{background:"#08090f",border:"1px solid rgba(255,255,255,.09)"}}>
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5" style={{background:"#090a13",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
            {[{icon:<Play size={10}/>,label:"Run Cell",onClick:nbRun,accent:true},{icon:<RotateCcw size={10}/>,label:"Clear",onClick:nbClear},{icon:<Sparkles size={10}/>,label:"AI Refine ✨",onClick:()=>{}}].map(b=>(
              <button key={b.label} onClick={b.onClick} className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-[5px] text-[10px] font-bold transition-all font-mono whitespace-nowrap ${b.accent?"text-[var(--accent)] hover:bg-[rgba(var(--ar),.2)]":"text-[#5a7080] hover:text-[var(--accent)] hover:border-[rgba(var(--ar),.22)]"}`}
                style={{background:b.accent?"rgba(var(--ar),.1)":"rgba(255,255,255,.035)",border:`1px solid ${b.accent?"rgba(var(--ar),.22)":"rgba(255,255,255,.07)"}`}}>
                {b.icon}{b.label}
              </button>
            ))}
