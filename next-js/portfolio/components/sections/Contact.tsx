...
import { Linkedin, Github, Mail, Copy, Play, RotateCcw } from "lucide-react";
...
export default function Contact() {
  ...
  const [output, setOutput] = useState<{ text: string; cls: "idle" | "running" | "ok" | "err" }>({
  ...
  function nbRun() {
    const newErrors: { name?: boolean; email?: boolean; msg?: boolean } = {};
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

    if (!name.trim()) newErrors.name = true;
    if (!email.trim() || !emailRegex.test(email)) newErrors.email = true;
    if (!msg.trim()) newErrors.msg = true;
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      let errText = `<span style="color: #f87171; font-weight: bold;">Traceback (most recent call last):</span>\n`;
      errText += `  File <span style="color: #8fb2ff;">"contact.ipynb"</span>, line <span style="color: #fbbf24;">1</span>, in <span style="color: #8fb2ff;">&lt;cell line: 1&gt;</span>\n`;
      errText += `    send_message(name="${name || "…'"}, email="${email || "…'"}, message="${msg ? "…" : "…'"}Ready")\n`;

      if (newErrors.name) {
        errText += `<span style="color: #f87171; font-weight: bold;">ValueError:</span> 'name' is a required argument and cannot be empty.\n`;
      }
      if (newErrors.email) {
        if (!email.trim()) {
          errText += `<span style="color: #f87171; font-weight: bold;">ValueError:</span> 'email' is a required argument and cannot be empty.\n`;
        } else {
          errText += `<span style="color: #f87171; font-weight: bold;">ValidationError:</span> '${email}' is not a valid email address.\n`;
        }
      }
      if (newErrors.msg) {
        errText += `<span style="color: #f87171; font-weight: bold;">ValueError:</span> 'message' is a required argument and cannot be empty.\n`;
      }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.4fr] gap-[28px] lg:gap-[44px] items-start">
        <div>
          <p className="text-[14px] leading-[1.7] text-[var(--muted)] mb-6 max-w-[380px]">
            {PERSONAL.summary}
          </p>
          <div className="flex flex-col gap-2.5">
            ...
            <button
              onClick={copyEmail}
              className="flex items-center gap-3 px-4 py-3 rounded-[12px] border text-[var(--accent)] font-bold text-[13px] text-left w-full transition-all hover:border-[rgba(143,178,255,.25)] hover:translate-x-1.5"
        ...
        {/* Jupyter Notebook */}
        <div
          className="rounded-[14px] overflow-hidden font-mono shadow-[0_28px_72px_-16px_rgba(0,0,0,.75)]"
          style={{ background: "#08090f", border: "1px solid rgba(255,255,255,.09)" }}
        >
          {/* titlebar */}
          <div
            className="flex items-center gap-2.5 px-4 py-2.5"
            style={{ background: "#0b0c15", borderBottom: "1px solid rgba(255,255,255,.05)" }}
          >
            ...
            <span className="flex-1 text-center text-[11px] text-[#3a4e62] tracking-[.4px]">
              contact.ipynb
            </span>
          ...
          <div
            ...
            style={{ background: "#090a13", borderBottom: "1px solid rgba(255,255,255,.04)" }}
          >
            {[
              ...
              <button
                ...
                className={`
                  ...
                  text-[10px] font-bold font-mono whitespace-nowrap transition-all cursor-pointer
                  ${
                    b.accent
                      ? "text-[var(--accent)] hover:bg-[rgba(143,178,255,.2)]"
                      : "text-[#5a7080] hover:text-[var(--accent)] hover:border-[rgba(143,178,255,.22)]"
                  }
                `}
                style={{
            ...
            <span className="ml-auto flex items-center gap-1.5 text-[10px] text-[#2a3a4a]">
              <span className="w-[7px] h-[7px] rounded-full bg-[var(--green)] shadow-[0_0_6px_rgba(74,222,128,.5)]" />
              Idle
            </span>
          </div>
          {/* cell */}
          <div
            ...
            style={{ borderColor: "rgba(255,255,255,.04)", background: "rgba(143,178,255,.015)" }}
          >
            <div className="absolute left-0 top-0 bottom-0 w-[2.5px] rounded-r bg-[var(--accent)] opacity-50" />
            <div
              className="w-[54px] flex-shrink-0 px-2 py-3 flex flex-col items-end"
              style={{ background: "#070810", borderRight: "1px solid rgba(255,255,255,.04)" }}
            >
              <span className="text-[11px] text-[var(--accent)]">[{count}]:</span>
            </div>
            <div className="flex-1 min-w-0">
              <div
                className="px-4 py-2.5 text-[11px] sm:text-[12px] leading-relaxed border-b overflow-hidden break-all whitespace-pre-wrap"
                style={{ background: "rgba(0,0,0,.15)", borderColor: "rgba(255,255,255,.04)" }}
              >
                <span className="text-[#f0b96a]">send_message</span>
                <span className="text-[#6a7e8e]">(</span>
                <span className="text-[var(--accent)]">name</span>=
                <span className="text-[#86c986]">"{previewName()}"</span>,{" "}
                <span className="text-[var(--accent)]">email</span>=
                <span className="text-[#86c986]">"{previewEmail()}"</span>,{" "}
                <span className="text-[var(--accent)]">message</span>=
                <span className="text-[#86c986]">"{previewMsg()}"</span>
                <span className="text-[#6a7e8e]">)</span>
              </div>
              <div className="px-4 py-3.5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-1.5">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className={`rounded-[6px] px-2.5 py-2 text-white font-mono text-[12px] outline-none transition-all w-full border ${
                        errors.name
                          ? "border-[#f87171] bg-[#f87171]/[0.03]"
                          : "border-white/[.07] focus:border-[rgba(143,178,255,.28)] focus:bg-[rgba(143,178,255,.04)]"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[9px] text-[#2e4255] uppercase tracking-[1.5px] font-bold">
                      email <span className="text-[var(--accent)] opacity-80">*</span>
                    </label>
                    <input
                      type="email"
                      value={email}
                  ...
                  <div className="sm:col-span-2 flex flex-col gap-1.5">
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        ...
                        if (errors.msg) setErrors((prev) => ({ ...prev, msg: false }));
                      }}
                      placeholder="Hey Chockalingam, let's discuss…"
                      rows={4}
          ...
          <div className="flex min-h-[44px]" style={{ background: "#060710" }}>
            <div
              className="w-[54px] flex-shrink-0 px-2 py-3 flex flex-col items-end"
              style={{ background: "#050610", borderRight: "1px solid rgba(255,255,255,.04)" }}
            >
              <span className={`text-[11px] ${output.cls === "ok" ? "text-[#3a5a3a]" : "text-[#1e2e3e]"}`}>
                [{output.cls === "idle" ? "" : count}]:
              </span>
            </div>
            <div
              className={`flex-1 px-4 py-3 text-[12px] leading-[1.8] whitespace-pre-wrap break-all ${outCls}`}
              dangerouslySetInnerHTML={{ __html: output.text }}
            />
          </div>
          {/* status bar */}
          <div
            className="flex items-center text-[9px] border-t"
            style={{ background: "#060710", borderColor: "rgba(255,255,255,.04)" }}
          >
            ...
            <span
              className={`ml-auto pr-3.5 py-[5px] ${
                output.cls === "running"
                  ? "text-[var(--amber)]"
                  : output.cls === "ok"
                  ? "text-[#2a4a2a]"
